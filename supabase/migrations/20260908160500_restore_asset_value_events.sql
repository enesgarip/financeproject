-- asset_value_events restore kapsamına girer (20260908160000 devamı).
--
-- `restore_user_finance_data_tx` tablo sırasını SABİT bir whitelist'te tutar
-- (utils/backup.ts RESTORE_TABLE_ORDER'ın SQL ikizi); yeni tablo listede
-- yoksa yedekteki satırlar "tanınmayan tablo" diye reddedilir. Fonksiyon
-- 20260826090000'deki sürümle BİREBİR aynıdır; yalnız:
--  * v_order'a 'assets'tan sonra (FK parent-first) 'asset_value_events' eklendi.
--  * assert_restored_parents_owned'a asset_value_events.asset_id kontrolü eklendi.
-- assets INSERT'i olay üretmez (trigger yalnız UPDATE'te) — restore edilen
-- olaylar çift sayılmaz.
...
begin
  ...
  if v_leak is null then
    select msg into v_leak from (
      select 'asset_value_events.asset_id' as msg from public.asset_value_events c
        join public.assets p on p.id = c.asset_id
        where c.user_id = p_user and p.user_id <> p_user limit 1
    ) q;
  end if;
