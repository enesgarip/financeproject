-- DH-02: Kaba transaction fingerprint'i yalnız mutabakat sinyali olarak bırak;
-- gerçek retry tekilleştirmesini kaynak olay kimliğiyle atomik olarak yap.

alter table public.card_expenses
  add column if not exists source_event_id text;

alter table public.card_expenses
  drop constraint if exists card_expenses_source_event_id_not_blank;

alter table public.card_expenses
  add constraint card_expenses_source_event_id_not_blank check (
    source_event_id is null or btrim(source_event_id) <> ''
  );

create unique index if not exists card_expenses_source_event_unique_idx
  on public.card_expenses (user_id, source, source_event_id)
  where source_event_id is not null;

drop function if exists public.add_card_expense(uuid, numeric, text, date, integer, text, text, uuid, text);
...
-- Eski 7 argümanlı fonksiyon eski istemciler için kalır. Yeni istemci olay
-- kimliğini zorunlu 8. argümanla gönderir; wrapper eski atomik işi çağırır.
...
begin
  ...
  if v_event_id is not null then
    ...
    from public.card_expenses
    where user_id = v_user_id and source = 'carryover' and source_event_id = v_event_id;
...
-- Aynı uyumluluk deseni planlı ödeme eşleştirmesinde de uygulanır.
...
begin
  ...
  if v_event_id is not null then
    ...
    select * into v_expense
    from public.card_expenses
    where user_id = v_user_id and source = v_source and source_event_id = v_event_id;
    if found then
