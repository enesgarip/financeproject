-- Asset buy/sell actions backed by a selected bank account.
--
-- A single user action updates the asset, updates the selected bank account
-- through the shared bank-account helpers, writes account_ledger via trigger,
-- and records one user-facing transaction_history row.
...
check (type in ('payment', 'transfer', 'loan', 'debt', 'card', 'correction', 'asset'));
...
begin
  ...
  if p_direction not in ('buy', 'sell') then
    raise exception 'Geçersiz varlık işlemi.';
  end if;

  if v_amount <= 0 then
    raise exception 'İşlem tutarı 0 dan büyük olmalı.';
  end if;
  ...
  if not found then
    raise exception 'Varlık bulunamadı.';
  end if;
  ...
  if v_asset.category = 'Hisse' and v_quantity is null then
    raise exception 'Hisse işlemlerinde adet girilmeli.';
  end if;
  ...
  set amount = v_next_amount,
      estimated_value_try = v_next_value,
  ...
  v_note :=
    case
      when p_direction = 'buy' then v_account.card_name || ' hesabından ödendi.'
  ...
  values (
    v_user_id,
    'asset',
    v_asset.name || case when p_direction = 'buy' then ' alındı' else ' satıldı' end,
    v_amount,
    'assets',
    v_asset.id,
    v_note
