-- Bir ekstre PDF'ini açık dönem için kaynak gerçek olarak uygular.
...
begin
  ...
  loop
    ...
    if v_kind = 'expense' then
      ...
      if found then
        if v_preserved_expense.installment_count <> v_total_installments
           or abs(v_preserved_expense.amount - round(v_installment_amount * v_total_installments, 2)) > 1 then
          raise exception 'Ödenmiş geçmişi olan taksit planının toplamı PDF ile uyuşmuyor; tarihsel parent otomatik değiştirilemez.';
        end if;
        ...
        loop
          ...
          insert into public.card_installments (
            ...
            null, 'Ekstre PDF kaynak gerçeğinden yeniden kuruldu.'
          );
