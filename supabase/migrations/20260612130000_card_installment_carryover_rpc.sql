-- Güven turu — Madde 3 (+ Madde 2 modeli): taksit devri atomik RPC.
--
-- "Taksit devri" akışı (CardsPage LegacyInstallmentPanel) önceden 4 ayrı client
-- çağrısıydı: card_expenses insert → card_installments insert (elle rollback) →
-- cards.debt_amount update → transaction_history insert (best-effort). Atomik
-- değildi; ara adımda hata olursa yarım kayıt kalabiliyordu. Bu RPC hepsini tek
-- transaction'da yapar (add_card_expense / record_manual_account_movement deseni).
--
-- Madde 2 modeli ("tüm taksitler borca dahil"): debt_amount kalan TÜM taksitlerin
-- toplamı kadar artar; current_period_spending yalnızca ilk taksit bu ay döneme
-- giriyorsa artar. cards.debt_amount değişimini A2 card_ledger trigger'ı
-- (cards_debt_ledger) aynı transaction içinde olaya çevirir.
...
begin
  ...
  -- Harcama kaydi (installment_amount Madde 1 trigger'i ile normalize edilir)
  ...
  -- Madde 2: kalan tum taksitler borca eklenir; bu ayki taksit doneme girer.
  -- Bu update A2 card_ledger trigger'ini ayni transaction'da tetikler.
  update public.cards
