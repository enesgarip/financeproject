-- stock_trades restore kapsamına girer (20260908180000 devamı).
--
-- restore_user_finance_data_tx sabit whitelist tutar (utils/backup.ts
-- RESTORE_TABLE_ORDER'ın SQL ikizi). Fonksiyon 20260908160500'deki sürümle
-- BİREBİR aynıdır; yalnız:
--  * v_order'a 'asset_value_events'tan sonra 'stock_trades' eklendi (FK asset_id).
--  * assert_restored_parents_owned'a stock_trades.asset_id kontrolü eklendi.
...
begin
  ...
  if v_leak is null then
    select msg into v_leak from (
      select 'stock_trades.asset_id' as msg from public.stock_trades c
        join public.assets p on p.id = c.asset_id
        where c.user_id = p_user and p.user_id <> p_user limit 1
    ) q;
  end if;
