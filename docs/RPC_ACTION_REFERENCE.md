...
| `replace_card_statement_import` | `replaceCardStatementImport` in `cardsRepo` | Statement PDF import | In one transaction preserves immutable history/later movements, replaces the PDF-covered open expense and installment scope, applies refunds, locks the cut amount to the bank total, cuts the statement, and records reconciliation. Date/row conflicts roll everything back. |
