...
| `update_card_expense` | `updateCardExpense` in `cardsRepo` | Cards page: edit an unstatemented posted expense | Reverses previous posted impact, writes new expense values, recreates installment rows. Rejects a directly archived expense or an installment parent with any archived child. On a bank-account (debit) card the sufficiency check runs against the balance AFTER the old amount is refunded (`20260812120000`); the earlier version compared a pre-refund snapshot and rejected legitimate edits that lowered the amount. pgTAP: `faz_f_db_fixes.sql`. |
| `post_card_provision` | `applyCardProvision` in `cardsRepo` | Cards page: post a provision | Moves all or part of a provision into posted current-period spending. Installment due dates derive from the transaction day (`spent_at + n months`, NOT month-start), every overdue installment is posted immediately, and current-period spending grows by the sum of overdue installments; `context_id` is preserved on partial posts (restored by `20260812090000` after the `20260811100000` regression, pgTAP: `provision_post_semantics.sql`) |
...
| `reset_card_import_data` | no TS caller since Faz C — DB-only maintenance helper (typed in `database.ts`; the import UI never exposed destructive clean import) | Guarded maintenance helper, run manually via SQL | Clears only the non-paid/open working scope under a user/card-bound DB context. It fails before mutation with an explicit message when current-settlement or paid-statement installment history would require reconstructing immutable evidence — those two preconditions were lost in `20260805120000` and restored in `20260812120000` (guard triggers blocked the delete either way, but the user only saw a generic trigger error). pgTAP: `faz_f_db_fixes.sql`. |
...
| RPC | Called From | User-Visible Action | Main Effect |
| --- | --- | --- | --- |
| `pay_payment` | `submitFinanceObligationPayment` | Planned payments page/dashboard obligation modal | Marks one payment paid or advances monthly recurrence; bank source debits `current_balance`, credit-card source increases `debt_amount` / `current_period_spending` and creates posted card spending; “Nakit / hesap dışı” writes history without changing a tracked balance |
| `pay_card_statement` | `submitFinanceObligationPayment` | Pay open credit-card statement | Uses the archived bank amount, debits the source account, marks the statement paid, reduces card debt, and reprojects statement debt from remaining open archives. It does not mutate linked installments and aggregate drift does not block payment — EXCEPT when the card's statement bucket is already zero: that means the archive's debt was settled through another flow (e.g. `pay_card_debt`), so the RPC refuses instead of debiting the bank a second time (`20260812091000`, pgTAP: `statement_double_payment_guard.sql`). Optional `p_skip_source_debit` (BM-3/B4): when a bank SMS already debited the source account, the statement is settled without a second debit. Optional `p_amount` (K7, `20260812110000`) enables partial/minimum statement payments: the amount is appended to `card_statement_payments`, the archive amount itself never changes, and the remaining balance is derived (`archive.statement_debt_amount − Σ payments`). A payment equal to the remaining balance (or an omitted `p_amount`) closes the archive; anything smaller leaves it open. The card's statement bucket is reprojected as the sum of open archives' REMAINING amounts. pgTAP: `partial_statement_payment.sql`. |
...
| `settle_personal_debt` | `submitFinanceObligationPayment` | Settle personal debt or collect receivable | Updates bank-account balance. Since BM-8 an optional `p_amount` allows a partial payment: null/full amount closes the debt row, a smaller amount reduces it and leaves the row open. Since `20260908170000` optional `p_skip_account_move` (drawer checkbox when a same-amount SMS movement exists on the account in the last 3 days) closes/reduces the record without touching the balance |
