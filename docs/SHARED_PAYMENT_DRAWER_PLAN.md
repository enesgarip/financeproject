...
| --- | --- | --- | --- | --- |
| Planned payment | `ObligationsCalendar` (and the dashboard Şerit row) -> `PaymentsPage.openObligationPayment` | `submitFinanceObligationPayment` -> `pay_payment` | Bank account or credit card | Amount is editable because estimates can be paid with the real amount. The payment record card itself has no "Öde" button: on a card-instructed row it contradicted the "this is informational, the SMS/statement posts it" note right above it. |
