...
| Expense cancelled (any status) | `cancel_card_expense` | `debt_amount -= amount`; `current_period_spending -= amount`; if provision then `provision_amount -= amount` too | Marks the expense `cancelled`; removes related installment rows; logs a correction to `transaction_history` |
...
| Card debt manual correction | `post_card_debt_correction` | `debt_amount += signed correction`; a *reduction* also peels the same amount off the visible split (current period first, then provision, then statement) so `current_period_spending` follows the total down; an *increase* leaves the split untouched | Writes an auditable `card_ledger.kind='adjustment'` event with the required reason note |
