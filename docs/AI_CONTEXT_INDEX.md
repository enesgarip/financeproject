...
| **Karar anı ("alsam mı")** | `utils/purchaseImpact.ts` (+ test; kartta ilk taksit sonraki ekstrede), `utils/cashFlowForecast.ts`, `utils/cardTimingChoice.ts` (+ test; kartlar arası kesim kıyas çipleri — hızlı harcama formu + karar ekranı) | `financeSnapshotRepo.ts` | `pages/PurchaseDecisionPage.tsx` (`/alsam-mi`, QuickActions ilk sırada) |
