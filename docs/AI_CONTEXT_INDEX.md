...
| **AI finans asistanı (sohbet)** | `utils/aiContext.ts` (+test; snapshot → kompakt Türkçe özet, öncelikli bölümler, composite/çıpalı hedefte TL basılmaz), `app/useAiChat.ts` (gönderim/retry sözleşmesi: invoke düşerse user mesajı DB'de kalır, retry text'siz) | `data/repositories/aiChatRepo.ts` (tek akış, thread yok), `lib/aiChatClient.ts`, `supabase/migrations/20260829100000_ai_chat_messages.sql` (own-row RLS, UPDATE yok, backup kapsamı DIŞI) | `pages/AssistantPage.tsx` (`/analiz/asistan`; Şerit transkript, balonsuz), `supabase/functions/ai-chat` (Gemini Flash; verify_jwt varsayılan true, DB'ye dokunmaz, `GEMINI_API_KEY` paylaşık) |
