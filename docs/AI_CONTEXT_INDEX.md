...
| `/analiz/asistan` AI asistan | `src/pages/AssistantPage.tsx` | `src/app/useAiChat.ts`, `src/data/repositories/aiChatRepo.ts`, `src/lib/aiChatClient.ts`, `src/utils/aiContext.ts`, `src/hooks/useMarketRates.ts` + `src/hooks/useSafeToSpend.ts` (kur + kasa kovaları → hedef türetimi), `supabase/functions/ai-chat` |
