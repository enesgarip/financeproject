...
| **Ekstre / statement döngüsü** | `utils/cardStatement.ts`, `utils/statementCycle.ts`, `utils/statementReminder.ts`, `utils/importReviewPeriod.ts`, `utils/statementReconcileReview.ts`, `utils/statementImportPlan.ts` (+ test; payment/carryover/expense/**needs-review** kararı), `utils/denizBankStatementParser.ts` (paid child cari aday değildir; strict sıra+tarih adayı açıklama uyuşmazsa tutar yakınlığı zorunlu; parent yalnız PDF no/adedi birebir child'dan reuse edilir), `utils/yapiKrediStatementParser.ts`, `utils/importedInstallmentPlan.ts`, `utils/transactionFingerprint.ts` | `data/repositories/cardsRepo.ts` (`replaceCardStatementImport`: PDF tarihine kadar açık scope'u atomik yeniden kurar; paid/current-settled geçmiş + sonraki hareket korunur), `services/cardLedgerActions.ts`, `supabase/migrations/20260806120000_reconcile_card_installment.sql`, `supabase/migrations/20260809160000_allow_statement_installment_total_drift.sql` (tarihsel parent tutar farkı kabul; adet farkı blok), `supabase/tests/statement_import_installments.sql` (1./orta/son taksit + parent toplam sapması + paid-history/later-movement preserve + reimport idempotency + rollback; `npm run db:test:statement-import`) | `components/finance/StatementImportModal.tsx` (PDF authoritative replacement; tüm satırlar zorunlu, belirsiz taksit önce doğrulanır), `pages/CardsPage.tsx`, `pages/CardsPage.statements.tsx` |
