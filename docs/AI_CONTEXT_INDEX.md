...
| **Şerit görsel dili (kartsız)** | `utils/dashboardMonthStrip.ts` (+ test; ay şeridi türevi), `utils/formatCurrency.ts` (`formatSeritParts`, sembol sonda) | — | `components/serit/*` (ortak parçalar; Card bileşeni YOK), `components/dashboard/SeritOverview.tsx`, `components/dashboard/SeritMonthStrip.tsx`, `src/index.css` (`--page`/`--ink*`/`--line*`/`--signal-*` tokenları) |
