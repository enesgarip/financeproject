...
| **Bütçe & birikim hedefleri (planlama)** | `utils/savingsGoal.ts`, `utils/budgetAlerts.ts`, `utils/goalSnapshots.ts` (+test; günlük hedef fotoğrafına girecek satırlar — tempo/varış tahmini tarihçesi, tutar hedefin KENDİ biriminde) | `data/repositories/savingsGoalsRepo.ts`, `data/repositories/analysisRepo.ts` (`recordSavingsGoalSnapshots`), `supabase/migrations/20260825090000_savings_goal_snapshots.sql` (+`supabase/tests/savings_goal_snapshots.sql`) | `pages/PlanningHub.tsx`, `pages/PlanningPage.tsx`, `components/finance/SavingsGoalsPanel.tsx`, `app/useDailyNetWorthSnapshot.ts` + `app/dailyGoalSnapshots.ts` (günlük yazım; gövde lazy modülde — entry bütçesi) |
