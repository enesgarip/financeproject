...
| **Ekstre / statement döngüsü** | `utils/cardStatement.ts`, `utils/statementCycle.ts`, `utils/statementReminder.ts`, `utils/importReviewPeriod.ts`, `utils/denizBankStatementParser.ts`, `utils/importedInstallmentPlan.ts` (+ testler), `utils/transactionFingerprint.ts` | `data/repositories/cardsRepo.ts` (`fetchCardExpenseMatchRows`, `fetchCardInstallmentMatchRows` archive marker taşır; installment amount mismatch otomatik write yapmaz, `fetchCardPaymentMatchRows`, `addCardExpense`, `recordCardInstallmentCarryover`, `payPaymentFromCardImport`), `supabase/tests/card_allocation_security.sql` | `components/finance/StatementImportModal.tsx`, `pages/CardsPage.tsx`, `pages/CardsPage.statements.tsx` (StatementArchivePanel — arşiv Ekstreler sekmesinde) |
