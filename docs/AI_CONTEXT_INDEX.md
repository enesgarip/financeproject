...
| --- | --- | --- | --- |
| Card debt fields and transitions | `docs/CARD_DEBT_TRANSITIONS.md` | `src/utils/financeSummary.ts`, `src/pages/CardsPage*.tsx`, `src/components/finance/LiveReconciliationPanel.tsx`, `src/services/cardLedgerActions.ts`, latest card migrations, `supabase/tests/legacy_current_payment_allocation.sql`, `supabase/tests/provision_statement_cut_order.sql` (bakım sırası: stale provizyon post → kesim) | Use this before touching `debt_amount`, `statement_debt_amount`, `current_period_spending`, `provision_amount`, card installments, bank-snapshot reconciliation, or historical payment allocation |
...
| Shared payment drawer | `docs/archive/SHARED_PAYMENT_DRAWER_PLAN.md` | `src/hooks/useFinancePaymentDrawer.ts`, `src/components/finance/FinancePaymentDrawer.tsx`, `src/components/finance/AccountPaymentModal.tsx`, `src/services/financePaymentActions.ts`, payment-owning pages | Plan and shared implementation for account-backed payment modals without changing RPC behavior |
...
| Banking simplification | `docs/archive/BANKING_SIMPLIFICATION_AUDIT.md` | `src/pages/CardsPage*.tsx`, `src/components/finance/*`, `docs/CARDS_ARCHITECTURE.md` | Tracks completed banking simplifications and future maintenance notes |
...
| Components | `src/components/*` | Reusable UI, finance panels, charts (saf SVG: BarChart, LineChart + chartUtils; parça-bütün için `CompositionBar`), selectors, modals. |
...
| `/varliklar` assets | `src/pages/AssetsPage.tsx`, `src/pages/AssetsPage.tradeModal.tsx`, `src/pages/AssetsHub.tsx`, `src/pages/BorsaPage.tsx` (`/varliklar/borsa`), `src/pages/CarsPage.tsx` (`/varliklar/araclar`) | `src/data/repositories/valuationRepo.ts`, `src/data/repositories/assetValueEventsRepo.ts`, `src/services/assetTrades.ts`, `src/services/assetValueHistory.ts`, `src/components/finance/AssetValueHistoryPanel.tsx`, `src/utils/valuation*`, `src/utils/marketRates.ts`, `src/app/useCars.ts`, `src/data/repositories/carsRepo.ts` |
...
| `/analiz/asistan` AI asistan | `src/pages/AssistantPage.tsx` | `src/app/useAiChat.ts`, `src/data/repositories/aiChatRepo.ts`, `src/lib/aiChatClient.ts`, `src/utils/aiContext.ts`, `src/hooks/useMarketRates.ts` + `src/hooks/useSafeToSpend.ts` (kur, kasa, tampon), `src/app/useCars.ts` + `data/repositories/{analysisRepo,expenseContextsRepo,wishlistRepo}.ts` (sayfadaki `useAssistantExtras` best-effort ek verisi), `supabase/functions/ai-chat` |
...
| **Banka/nakit bakiyesi / hareket / mutabakat** | `utils/accountLedger.ts`, `utils/reconciliation.ts` (`STALE_AFTER_DAYS`=7; push-notify haftalık hatırlatma aynı eşiği kullanır. `cards.account_kind`: `bank` banka hesabı, `cash` fiziksel nakit cüzdanı; ikisi de `card_type=banka_karti` ile aynı ledger/RPC invariant'larını kullanır; migration/test: `20260918210000_cash_wallet_account_kind.sql`, `supabase/tests/cash_wallet.sql`. **Faz D1:** `drift` ham ölçümdür ve düzeltme sonrası DEĞİŞMEZ — DB'de `check (drift = app_amount - real_amount)`; "kapandı mı" sorusu `resolution` kolonunda: `matched`/`open`/`corrected`, NULL = eski kayıt. Durum türetirken `isUnresolvedDrift`'i kullan, `app_amount ≠ real_amount`'a BAKMA — düzeltilmiş kayıt sonsuza dek kırmızı kalır. `buildDriftHistory` sapma desenini verir), `utils/cardControlCenter.ts` (aynı invariant'ı paylaşır) | `data/repositories/financePanelsRepo.ts`, `services/accountLedgerActions.ts`, `services/accountMovements.ts` | `pages/CardsPage.tsx`, `pages/CardsPage.list.tsx`, `pages/CardsPage.control.tsx`, `components/finance/AccountSelector.tsx`, `components/finance/AccountLedgerPanel.tsx`, `components/finance/LiveReconciliationPanel.tsx`, `components/finance/CurrentMovementImportModal.tsx` |
...
| **Kişisel borç/alacak** | `utils/obligations.ts` | `services/financePaymentActions.ts` (settle/collect kısmi ödeme; `findRecentSmsAccountMovement` yönlü SMS hareketi araması), `supabase/migrations/20260810190000_partial_personal_debt_and_due_collision.sql` (settle_personal_debt `p_amount` = kısmi; cut_card_statement vade çakışma ötelemesi), `supabase/migrations/20260908170000_sms_personal_debt_match.sql` (**SMS ↔ alacak/borç otomatik eşleme:** `private.fold_match_text` Türkçe aksan katlama, `private.match_open_personal_debt` — kayıttaki adın her kelimesi gönderici adında TAM kelime + aynı yön + TRY + tutar ≤ değer + max(5 TL, %1); tam olarak BİR aday yoksa tahmin yok, sebep `debt_note`; `private.settle_personal_debt_internal` ortak gövde; `settle_personal_debt` yeni `p_skip_account_move` = kart ödemesindeki `p_skip_source_debit` ikizi; `record_sms_account_movement` artık **jsonb** döner: kart alanları + `matched_debt` + `debt_note` + `replayed`), `supabase/tests/partial_debt_and_due_collision.sql`, `supabase/tests/sms_personal_debt_match.sql` | `pages/DebtsPage.tsx`, `LiabilitiesHub.tsx` |
| **Planlı ödemeler** | `utils/dashboardUpcoming.ts`, `utils/attention.ts`, `utils/financeObligationRules.ts`, `utils/paymentHistory.ts`, `utils/paymentEstimate.ts` (+test; `amount_status='estimated'` satırın tutarını gerçekleşen ödemelerin MEDYANIyla tazeleme ÖNERİSİ — otomatik yazmaz, tutar kullanıcının taahhüdüdür; eşikler %10 VE 25 TL, örnek < 2 ise susar) | `services/financePaymentActions.ts` | `pages/PaymentsPage.tsx` (kartta "Güncelle" tek tık) |
| **Manuel varlık değer geçmişi (BES/Araç/Fon/Diğer)** | `utils/assetValueEvents.ts` (+test; olay = önceki→yeni değer kuruş + dönem katkısı; **getiri = Δdeğer − katkı**, yüzde tabanı dönem başı + katkı; toplam getiri satır toplamıdır, uç fark değil — kopuk zincir hayalet kazanç üretmez) | `data/repositories/assetValueEventsRepo.ts`, `services/assetValueHistory.ts` (`update_asset_value` RPC: değer + katkı + tarih + not TEK transaction; akış satırı pozitif tutarlı, yön başlıkta), `supabase/migrations/20260908160000_asset_value_events.sql` (tablo + `assets` AFTER UPDATE trigger'ı `record_asset_value_event` — yalnız iki tarafı da manuel satır, `app.asset_event_suppress` GUC'u; eski `₺a → ₺b` başlıklı geçmiş satırları taşındı), `supabase/migrations/20260908160500_restore_asset_value_events.sql` (restore whitelist), `supabase/tests/asset_value_events.sql`. Eski yol (`recordAssetValueChange`, client'tan işaretli delta) KALDIRILDI: `transaction_history.amount >= 0` kısıtı düşüşleri sessizce yutuyordu | `components/finance/AssetValueHistoryPanel.tsx` (varlık kartında; başlıkta "İlk kayıt X · Katkı · Getiri (%)", satırda dönem → önceki→yeni · katkı · getiri; Tümü/12 ay/Bu yıl; "Değer güncelle" formu) |
...
| **Birikim hedefleri** | `utils/savingsGoal.ts`, `utils/savingsSuggestion.ts` (aylık gerekli + nakit-akışı tavsiyesi). **DİKKAT — karma (composite) hedefte `target_amount`/`current_amount` TL DEĞİL, bileşen sayısıdır**: toplam bileşen ve hedefine ulaşan bileşen. Bunları para gibi biçimlendirme/toplama (`formatAmount`, `diffTL`) — `formatSavingsGoalAmount` kullan ya da composite'i ayıkla (`savingsSuggestion.ts`, `analysisView.buildSearchItems` guard'lı). Hedef matematiği TEK yerde: `savingsGoal.ts` + `savingsSuggestion.ts` — `financeSummary.ts`'te ikinci bir "aylık gerekli" hesabı açma, Faz D2'deki hata tam olarak iki implementasyondan birinin guard'sız kalmasıydı. **Faz D2:** sayaçlar `upsert_savings_goal` içinde `p_components`'tan türetilir, client'tan gelen değer yok sayılır — client'ta ikinci bir hesap TUTMA. **Takip kaynağı (2026-08-24):** hedef ya da bileşen `savings_goal_sources` üzerinden varlığa/kategoriye/tüm varlıklara/banka hesabına/kasa kovasına bağlanabilir; bağlı satırın `current_amount`'ı DB'de 0'dır ve tutar OKUMA ANINDA `utils/goalSources.ts` ile türetilir (canlı fiyat client'ta olduğu için DB hesaplayamaz). Hedef okuyan yeni bir yüzey açarsan ham satırı değil `resolveSavingsGoalRows` çıktısını kullan | `utils/goalSources.ts`, `utils/goalTargetAnchor.ts` (hedef TUTARININ çıpası: altın/dolar endeksi ya da "N aylık gider"; çıpalıda target_amount DB'de 0, tutar okuma anında türetilir — TÜFE kasıtlı olarak yok), `data/repositories/savingsGoalsRepo.ts`, `supabase/migrations/20260824100000_savings_goal_sources.sql`, `supabase/migrations/20260811150000_composite_goal_totals_from_components.sql`, `supabase/tests/composite_goal_totals.sql` | `components/finance/SavingsGoalsPanel.tsx` (Assets/Dashboard); surplus `pages/PlanningPage.tsx`'ten geçer |
| **Bütçe uyarıları** | `utils/budgetAlerts.ts` | `data/repositories/crudRepo.ts` | `pages/CardsPage.tsx` |
| **Dashboard özet/insight** | `utils/dashboardInsights.ts`, `utils/cashFlowForecast.ts`, `utils/dashboardUpcoming.ts`, `utils/obligations.ts`, `utils/financeObligationRules.ts`, `utils/netWorthSeries.ts`, `utils/dataHealthSummary.ts` | `data/repositories/financeSnapshotRepo.ts` (açılış snapshot'ı TEK `fetch_finance_snapshot` RPC'siyle iner — 17 sorgu yalnız RPC'siz ortam fallback'i; migration `supabase/migrations/20260824220000_fetch_finance_snapshot_rpc.sql`, sözleşme testi `supabase/tests/finance_snapshot_rpc.sql`. Pencere/sıralama değişecekse RPC gövdesi + legacy yol BİRLİKTE değişir) | `pages/DashboardPage.tsx` |
...
| **Aktivite akışı (audit trail)** | `utils/activityFeed.ts` ("Tümü"de otomatik ledger yankısı gizlenir), `utils/historyText.ts` (SQL'in ASCII geçmiş metnini ekranda Türkçeleştirir) | `data/repositories/financePanelsRepo.ts` | `pages/AnalysisPage.activity.tsx` (ActivityFeedPanel) |
...
| **Kredi kartı borcu (Borçlar bağlamı)** | `utils/financeSummary.ts` (`cardPayableDebt`) | `hooks/useFinancePaymentDrawer.ts` | KALDIRILDI (2026-09-08): `/borclar/kartlar` (`LiabilitiesCardsPage`) yetim kalmıştı — UI-1 sekmeyi Hesaplar'ın kart bölümüne yönlendirmişti; sayfa silindi, rota `/kartlar?section=kartlar`'a yönlenir. Kart borcu tek yüzeyden (`pages/CardsPage.*`) gösterilir ve ödenir |
| **Kart tutarlılık skoru** | `utils/cardConsistency.ts` (+ test) | — | `pages/CardsPage.list.tsx` (skor badge) |
| **Taksit ufku (karar anı + bitişler)** | `utils/installmentHorizon.ts` (+test; `buildPlannedInstallmentHint` "aylık yükün ₺X→₺Y" — taban önümüzdeki ay; `findInstallmentReliefs` plan bitişleri — yalnız 'scheduled', dil "taksit yükü" nakit değil) | — | `components/finance/CardInstallmentIntentPanel.tsx` (niyet formu ipucu), `pages/CardsPage.statements.tsx` (ProvisionPanel taksit seçimi altı), `components/finance/CardInstallmentCalendarPanel.tsx` ("Yaklaşan bitişler") |
| **Ekstre tahmini (kesime az kala)** | `utils/statementEstimate.ts` (+test; ≤5 gün kala "kapanırsa ~₺X" = dönem içi kova + o ekstre ayının planlı taksitleri; "~" bilinçli — provizyon/faiz bilinemez), `utils/statementProjection.ts` (+test; önümüzdeki 3 kesim dönemi "bilinen yükler" — k=0'da E3 formülü, abonelik EKLENMEZ (çift sayma); k≥1'de taksit + kart abonelik medyanı + karta talimatlı aylık ödeme; tempo tabanı toplam DIŞI ayrı satır) | — | `pages/CardsPage.list.tsx` (kredi kartı satırında CardDatum; detay genişlemesinde `components/finance/CardStatementProjectionPanel.tsx` — opt-in, E3'ün ≤5 gün pencere disiplinine bilinçli saygı) |
| **Dönem harcama temposu** | `utils/statementPace.ts` (+test; cari dönem vs önceki dönemin aynı gün-offset'i, iki yaka da card_expenses'ten — borç kovasıyla kaynak karıştırılmaz; önceki dönem boşsa susar) | `data/repositories/cardsRepo.ts` (`fetchExpensePaceRows`: son 70 gün hafif satırlar, tek toplu sorgu) | `pages/CardsPage.list.tsx` (kredi kartı satırında nötr tek satır) |
| **Bütçe & birikim hedefleri (planlama)** | `utils/savingsGoal.ts`, `utils/budgetAlerts.ts` (`buildBudgetUsage` maaş alır, çıpayı çözer), `utils/budgetAnchor.ts` (+test; limit çıpası avg_spend/salary_pct — çıpalı satırda limit 0 saklanır; bayat limit önerisi medyan+eşik; ay devri listesi), `utils/goalSnapshots.ts` (+test; günlük hedef fotoğrafına girecek satırlar — tempo/varış tahmini tarihçesi, tutar hedefin KENDİ biriminde), `utils/goalEta.ts` (+test; son 90 günden gerçekleşen tempo + varış tahmini — min 45 gün/2 ay gate'i, tempo ≤ 0'da tarih uydurulmaz), `utils/budgetBridge.ts` (+test; ayın son 3 gününde bütçe artığını en çok isteyen kovalı hedefe tek tık — localStorage ay damgalı) | `data/repositories/savingsGoalsRepo.ts`, `data/repositories/analysisRepo.ts` (`recordSavingsGoalSnapshots` + `fetchSavingsGoalSnapshots`), `supabase/migrations/20260825090000_savings_goal_snapshots.sql`, `supabase/migrations/20260825120000_budget_limit_anchor.sql` (+ikisinin `supabase/tests/*.sql` testleri) | `pages/PlanningHub.tsx`, `pages/PlanningPage.tsx` (bütçe formu çıpa seçimi + canlı önizleme; tek-tık devir/güncelleme yazmaları), `pages/AnalysisPage.panels.tsx` (`BudgetProgress` rozet+şeritler), `components/finance/SavingsGoalsPanel.tsx`, `app/useDailyNetWorthSnapshot.ts` + `app/dailyGoalSnapshots.ts` (günlük yazım; gövde lazy modülde — entry bütçesi) |
| **Kasa modu (bucket → gerçek harcanabilir)** | `utils/kasaMode.ts` (+test; likit − rezerve), `utils/safeToSpend.ts` (`reserved` alanı), `utils/goalBucket.ts` (+test; hedefe bağlı kovanın aylık planı, "bu ay ayrıldı mı", kova hedefin KAYNAĞI mı; `dominantBucketGoal` — budgetBridge + Vazgeçme Kazancı'nın ortak "asıl beslenen kovalı hedef" seçicisi), `utils/foregoneGain.ts` (+test; Alsam mı? "vazgeçtim → hedefe ayır" önizlemesi — tempo gate'i geçilmemişse tarih uydurulmaz), `app/savingsGoalSnapshotsKey.ts` (paylaşılan snapshot sorgu anahtarı) | `data/repositories/kasaBucketsRepo.ts` (`contributeToGoalBucket` → RPC; artırım sunucuda), `supabase/migrations/20260728120000_add_kasa_buckets.sql`, `supabase/migrations/20260824140000_goal_bucket_link.sql` (hedef bağı + ay damgası), `supabase/tests/goal_bucket_contribution.sql` | `components/finance/KasaModuPanel.tsx` (PlanningPage), `components/finance/SavingsGoalsPanel.tsx` (hedef kartında "Ayır"), `hooks/useSafeToSpend.ts` (rezerve düşülür; `useKasaBuckets` kova listesini de verir) |
| **Alışveriş listesi** | `utils/wishlistPlan.ts` (+test; "ne zaman alabilirim" = tampon+kasa rezervi tabanını bozmadan fiyatı karşılayan ilk projeksiyon ayı, ve tutarın asıl beslenen hedefin kaç aylık payına denk geldiği — KASITLI olarak "hedefini N ay geciktirir" demez, parayı o hedeften kısacağın bilinemez; `dominantSavingsGoal` export'u karar ekranının emek çevirisiyle paylaşılır), `utils/wishlistAge.ts` (+test; 30 gün bekleme kuralı rozetleri — mevcut created_at/purchased_at'ten türetilir), `utils/purchaseEffort.ts` (+test; "bu tutar neye denk" — iş günü / ortalama çıkış günü / hedef payı) | `data/repositories/wishlistRepo.ts`, `hooks/useWishlistItems.ts` (paylaşılan sorgu + `WISHLIST_QUERY_KEY` — dışarıdan yazan bunu invalidate etmeli) | `pages/WishlistPage.tsx` (hesap tutmaz; snapshot + kasa rezervi + tampondan besleyip util'e sorar; satırdan `?tutar=` ile Alsam mı? köprüsü), `pages/PurchaseDecisionPage.tsx` ("Şimdilik listeye at" tek-tık otomatik adla yazar) |
...
| **Abonelik / sabit gider yönetimi** | `utils/subscriptions.ts` (+test; ≥3 ay + ±%15 medyan tespiti; `buildSubscriptionPaymentDraft` tek tık "Planla" → bank_auto+kart bağlı aylık ödeme, yalnız kredi kartı kaynaklı — çift sayma yok; `subscriptionAlreadyPlanned` rozeti) | — | `pages/AnalysisPage.panels.tsx` (SubscriptionsPanel; yazma `saveCrudRow('payments')`) |
...
| **Yedek / backup** | `utils/backup.ts` (+ `backup.test.ts`, `backup.restore.test.ts`; ledgers, operasyon logları ve immutable current-settlement kanıtı export-only; alias/dismiss/push/wishlist/kasa/notification preferences restore edilir; malformed rows reset öncesi reddedilir; **restore RPC-öncelikli** — `restoreViaTransactionalRpc` null dönerse (RPC deploy değil) eski REST replay fallback'i) | `data/repositories/backupRepo.ts` (keyset export + transactional reset RPC + `restoreViaTransactionalRpc`), `supabase/migrations/20260826090000_transactional_restore.sql` (**tek-transaction restore**: reset + parent-first replay + mevcut-kolon insert (eski yedekte DEFAULT korunur) + settlement sıyırma + çapraz-parent sahiplik doğrulaması; kanıt `supabase/tests/transactional_restore.sql`), `supabase/migrations/20260802170000_complete_backup_reset_scope.sql` (reset kapsamı) | (DataHealth) |
| **İstemci hata izleme (Sentry'siz)** | `lib/errorReport.ts` (yazar — AppErrorBoundary + window error/unhandledrejection; oturum başına ~5 kayıt tavanı + hash'le günlük dedupe + 90 gün istemci-retention; `__APP_COMMIT__` damgası vite define'dan, build'de GITHUB_SHA) | `data/repositories/clientErrorsRepo.ts` (okuma + temizleme), `supabase/migrations/20260826180000_client_errors.sql` (own-row RLS, user_id default auth.uid(), UPDATE bilinçli yok) | `components/finance/ClientErrorsPanel.tsx` (`/veri-sagligi/islemler`), `components/AppErrorBoundary.tsx` (dinamik import), `src/main.tsx` (PROD'da gecikmeli kurulum — entry bütçesi) |
| **Push bildirim** | `utils/notificationPreferences.ts` (+test; tür→tercih + sessiz saat, edge ikizi; `quietHoursMuteDailyPush` — cron TEK saat (07:00) çalıştığı için o saati kapsayan sessiz aralık pratikte "kapalı" demektir, UI uyarır), `utils/salaryDeposit.ts` (+test; maaş yatışı tespiti ±%10 — edge ikizi + PlanningPage maaş şeridi; `findSalaryChangeCandidate` maaş değişikliği önerisi — transfer bacağı elemeli, tek tık salary_history satırı) | `data/repositories/pushSubscriptionsRepo.ts`, `data/repositories/notificationPreferencesRepo.ts` (tercih upsert + son gönderim), `services/pushNotifications.ts`, `lib/pushClient.ts` (`syncPushSubscription` + `shouldSyncPushSubscription`; 410/endpoint rotasyonuyla ölen aboneliği sessizce onarır — opt-out bayrağı olmadan kullanıcı bildirimleri kapatamaz), `hooks/usePushSubscriptionSync.ts` (açılışta onarım), `supabase/migrations/20260617102826_add_notification_log.sql`, `supabase/migrations/20260729120000_add_notification_preferences.sql`, `supabase/migrations/20260819100000_provision_installment_reminder_pref.sql` (`provisions_enabled`), `supabase/migrations/20260824200000_goal_contribution_reminder_pref.sql` (`goals_enabled`), `supabase/migrations/20260826150000_sms_alerts_pref.sql` (`sms_alerts_enabled`), `.github/workflows/push-notify.yml` | `supabase/functions/push-notify` (tercih/sessiz-saat kapısı; `provision_installment_pending`, `provision_statement_cut_risk` — kesime ≤2 gün kala hâlâ provizyonda olan ve kesim koşusuna dek stale eşiğini dolduramayacak işlemler, kart+kesim tarihi dedupe'uyla ekstre başına tek gönderim, provisions_enabled'a bağlı — ve `goal_contribution_due` adayları — ikincisi ayın 2-5'inde YA DA maaş yatışı tespit edildiyse 2-12'de, hedefe bağlı kovaya o ay ayırma yapılmadıysa; yatışta metin "maaş yattı görünüyor" olur; bildirimde TUTAR YOK çünkü kaynağa bağlı hedefin current_amount'ı DB'de 0'dır; `card_statement_cut_receipt` — kesim sabahı KESİNLEŞEN ekstre tutarı + vade, 48 saat pencere sessiz-saat telafisi, dedupe archive.id, statements_enabled'a bağlı; `sms_failure_daily` — son 26 saatte status='error' sms_log satırı varsa günde tek toplu uyarı, NULL user satırları SMS_OWNER_USER_ID'ye sayılır, parse-sms artık hata satırlarına da sahip damgası basar ki RLS'li panel gösterebilsin), `public/sw.js`, `components/finance/NotificationSettings.tsx` (tür toggle + sessiz saat + son gönderim) |
| **SMS harcama otomasyonu** | `utils/smsParser.ts` (+test; canlı edge parser aynası, DenizBank format/tutar/+03:00; gelen FAST/HAVALE/EFT SMS'inde gönderici adı `counterparty` — kişisel alacak eşlemesinin girdisi, bkz. "Kişisel borç/alacak" satırı), `utils/clipboardExpense.ts` (+test; "Panodan doldur" — pano metnini SMS aynası + TL-tutar fallback'ıyla form ön-doldurmasına çevirir; kart SMS'i source='sms' + webhook hash şeması → çift yönlü DB dedupe), `utils/categories.ts` (inferExpenseCategory), `utils/sourceEventId.ts` (artefakt hash / import satır kimliği), `utils/subscriptions.ts` (ödeme-planına bağlı kart hareketini abonelikte ikinci kez saymaz) | `data/repositories/cardAliasesRepo.ts`, `data/repositories/smsLogRepo.ts` (`fetchUnrecognizedSmsLog`), `supabase/migrations/20260626120000_add_sms_log.sql`, `supabase/migrations/20260702120000_tolerant_sms_account_matching.sql` (hesap no toleranslı eşleşme), `supabase/migrations/20260802120000_sms_account_movement_use_occurred_at.sql` (occurred_at = gerçek SMS zamanı), `supabase/migrations/20260802140000_card_expense_source_event_id.sql` (kart retry idempotency), `supabase/migrations/20260802180000_sms_account_movement_idempotency.sql` (hesap retry idempotency + service-role-only RPC), `supabase/migrations/20260809200000_reconcile_sms_card_auto_payments.sql` (kart SMS'i ↔ otomatik ödeme planı çapraz-kaynak tekilleştirme), `supabase/tests/sms_card_payment_reconciliation.sql` | `supabase/functions/parse-sms`, `components/finance/CardAliasPanel.tsx`, `components/finance/SmsLogPanel.tsx` (`/veri-sagligi/islemler`) |
...
| **Şerit görsel dili (kartsız)** | `utils/dashboardMonthStrip.ts` (+ test; ay şeridi türevi), `utils/formatCurrency.ts` (`formatSeritParts`/`formatSeritAmount`, sembol sonda — `useBalancePrivacy` bunu kullanır) | — | `components/serit/*` (ortak parçalar: `ScreenHeader`/`HeroNumber`/`LineGroup`/`LineRow`/`Divider`/`BreakdownBar`/`TrendBars`/`Delta`/`tone.ts`; kart bileşeni burada değil `components/ui/card.tsx`te), `components/Layout.tsx` + `components/BottomNav.tsx` + `components/QuickActions.tsx` (kabuk `4e`, FAB 76px ayrılmış bant), `components/HubNav.tsx` (2px jade sekme şeridi), `components/dashboard/Serit*.tsx`, `pages/AnalysisPage.hero.tsx`, `pages/CardsPage.hero.tsx`, `src/index.css` (`--page`/`--ink*`/`--line*`/`--shadow-card`/`--warning-fill`/`--color-viz-*` + `.serit-num`/`.serit-eyebrow`) |
| **Karar anı ("alsam mı")** | `utils/purchaseImpact.ts` (+ test; kartta ilk taksit sonraki ekstrede), `utils/cashFlowForecast.ts`, `utils/cardTimingChoice.ts` (+ test; kartlar arası kesim kıyas çipleri — hızlı harcama formu + karar ekranı) | `financeSnapshotRepo.ts` | `pages/PurchaseDecisionPage.tsx` (`/alsam-mi`, QuickActions planlama kısayolu) |
...
| **Hisse fiyatları (canlı + tarihsel)** | `hooks/useStockPrices.ts`, `lib/stockQuotesClient.ts` (`fetchStockPrices` son fiyat; `fetchStockHistory(symbols, range)` günlük kapanış serisi — sembol başına en geniş pencere 24 saat cache) | — | `pages/AssetsPage.tsx` (Hisse satırları), `pages/BorsaPage.tsx`, `supabase/functions/bist-quote` (`range` beyaz listesi 1mo…max → `history[symbol] = {t, c}` aynı Yahoo yanıtından; sembol çağrıları 4 işçiyle sınırlı, `historyVersion: 1`) |
| **Hisse işlem defteri / portföy performansı** | `utils/stockLedger.ts` (+test; ağırlıklı ortalama maliyet havuzu — satış o anki ortalamadan düşer ve farkı GERÇEKLEŞMİŞ K/Z yazar; fiyatsız açılış adette sayılır, maliyet tabanına girmez; `stockPeriodPerformance` NAKİT-AKIŞ DÜZELTİLMİŞ: kazanç = V1 − V0 − alımlar + satışlar, taban V0 + alımlar; `closeOnOrBefore`, `historyRangeFor`, `stockLedgerDrift` defter↔varlık adet farkı) | `supabase/migrations/20260908180000_stock_trades.sql` (tablo: symbol/kind buy-sell-opening/trade_date/quantity/unit_price nullable/fee/source; mevcut Hisse satırlarından `opening` backfill; `trade_asset_with_account` Hisse+miktarlı işlemde aynı transaction'da satır yazar, birim fiyat = nakit/adet), `20260908180500_restore_stock_trades.sql` (restore whitelist), `20260908180700_reset_stock_trades.sql` (reset kapsamı — FK set null olduğu için cascade yok), `supabase/tests/stock_trades.sql`. **assets Hisse satırı KANONİK pozisyon kalır** (hedef kaynağı, net değer, canlı senkron); defter tarihçedir, elle girilen satır nakit/varlığa dokunmaz; uyuşmazlık sayfada uyarı, otomatik düzeltme YOK | `pages/BorsaPage.tsx` (`/varliklar/borsa`: hero portföy değeri, baştan beri Delta, maliyet/gerçekleşmiş/gerçekleşmemiş, dönem seçici Baştan beri/1A/3A/Bu yıl/1Y/Özel, sembol satırları, CrudPage defter listesi + "Geçmiş işlem ekle"), `pages/AssetsPage.tradeModal.tsx` (satış önizlemesi artık RPC ile aynı oransal model) |
...
| **AI finans asistanı (sohbet)** | `utils/aiContext.ts` (+test; snapshot → kompakt Türkçe özet ≤16k, öncelik sıralı ~24 bölüm: pozisyon, bu/gelecek ay, güvenle-harcanabilir, kartlar+mutabakat tazeliği, limit grupları, taksit takvimi, ekstre geçmişi, krediler+bitiş, abonelik radarı, hedefler+ETA, net değer trendi, kasa/araç/bağlam/alsam-mı, trend, son hareketler; türetme HEP ekranların util'ine delege; kur/kova/tampon gibi yan girdi verilmeden çözülemeyen değer rakamsız düşer, liste kesikleri "… ve N daha" ile söylenir), `app/useAiChat.ts` (gönderim/retry sözleşmesi: yanıt düşerse user mesajı DB'de kalır, retry text'siz; onChunk canlı metni taşır) | `data/repositories/aiChatRepo.ts` (tek akış, thread yok), `lib/aiChatClient.ts` (STREAMING: invoke değil aynı-origin ham fetch + SSE; `{text}/{done}/{error}` sözleşmesi, done'suz biten akış = hata, kısmi metin kaydedilmez), `supabase/migrations/20260829100000_ai_chat_messages.sql` (own-row RLS, UPDATE yok, backup kapsamı DIŞI) | `pages/AssistantPage.tsx` (`/analiz/asistan`; Şerit transkript, balonsuz, canlı büyüyen yanıt bloğu), `supabase/functions/ai-chat` (Gemini Flash `streamGenerateContent` SSE proxy'si; verify_jwt varsayılan true, DB'ye dokunmaz, `GEMINI_API_KEY` paylaşık) |
...
- `CardsPage.sections.tsx`: iki büyük seçim alanı; `section=kartlar` yalnız kredi kartları, `section=hesaplar` banka hesapları + nakit cüzdanları. `CardsPage.hero.tsx` bölüm toplamını gösterir.
