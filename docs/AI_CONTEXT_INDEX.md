...
| `/analiz` reports hub | `src/pages/AnalysisHub.tsx`, `src/pages/AnalysisPage.tsx`, `src/pages/AnalysisDetailPage.tsx`, `src/pages/AnalysisPage.data.ts`, `src/pages/AnalysisPage.panels.tsx`, `src/pages/AnalysisPage.reports.tsx`, `src/pages/AnalysisPage.trends.tsx`, `src/pages/AnalysisPage.wealth.tsx`, `src/pages/CarsPage.tsx` (`/analiz/araclar`) | `src/app/useFinanceSnapshot.ts`, `src/app/useCars.ts`, `src/data/repositories/analysisRepo.ts`, `src/data/repositories/carsRepo.ts`, `src/utils/analysisView.ts`, charts |
...
| **Ekstre / statement döngüsü** | `utils/cardStatement.ts`, `utils/statementCycle.ts`, `utils/statementReminder.ts`, `utils/importReviewPeriod.ts`, `utils/statementReconcileReview.ts` (+ test; app-fazla tespiti + banka-toplamı kilit farkı/notu), `utils/statementImportPlan.ts` (+ test; SAF taksit import kararı — hangi satır payment/carryover/expense/**needs-review** olur; belirsiz adet veya tutarsız notasyon sessizce tek-harcama YAZILMAZ, manuel doğrulamaya düşer; modaldaki 3 tekrarın tek kaynağı), `utils/denizBankStatementParser.ts` (eşleşme tuning'i `importMatch.ts`'ten — provizyon dedup penceresi movement ile ortak; taksit notasyonundaki `kalan borç`u `ParsedTransaction.remainingDebt`'e okur; `checkStatementParseTotals` = PDF iç-tutarlılık checksum'ı: başlık kimliği + satır toplamı, 8 gerçek ekstrede kalibre, düşen/yanlış-okunan satırı yakalar — SI-04 app-vs-banka kilidinden FARKLI), `utils/yapiKrediStatementParser.ts` (+ test; YapıKredi/Worldcard cihaz-içi parser — Türkçe ay-isimli tarih, alt-satır taksit notasyonu, İşlem tablosu bölge sınırı; ParsedStatement döner, pipeline'ı DenizBank ile paylaşır), `utils/importedInstallmentPlan.ts` (+ testler; `checkInstallmentNotation`: kalan-borç ↔ `aylık × (toplam−sıra)` tutarlılık invariant'ı — yanlış adet okumasını yakalar), `utils/transactionFingerprint.ts` | `data/repositories/cardsRepo.ts` (`fetchCardExpenseMatchRows`, `fetchCardInstallmentMatchRows` archive marker taşır; installment amount mismatch otomatik write yapmaz, `fetchCardPaymentMatchRows`, `fetchCardById` import sonrası taze borç, `addCardExpense`, `recordCardInstallmentCarryover`, `cancelCardExpense` app-fazla iptali, `payPaymentFromCardImport`), `services/cardLedgerActions.ts` (`postCardDebtCorrection` kilit ters-kaydı), `supabase/tests/card_allocation_security.sql`, `supabase/tests/statement_import_installments.sql` (1. taksit / plan-ortası / son taksit DB regresyonu; `npm run db:test:statement-import`, CI+deploy DB gate'inde) | `components/finance/StatementImportModal.tsx` (Faz 1 banka-toplamı kilit / Faz 2 app-fazla iptal / Faz 3 inline manuel taksit), `pages/CardsPage.tsx`, `pages/CardsPage.statements.tsx` (StatementArchivePanel — arşiv Ekstreler sekmesinde) |
| **DenizBank güncel hareket mutabakatı** | `utils/denizBankMovementParser.ts` (+ `.test.ts`; eşleşme tuning'i `importMatch.ts`'ten), `utils/importMatch.ts` (+ test; movement+ekstre matcher'ın ORTAK tolerans/pencere/seçim kaynağı — tarih 7 gün, tutar max(5TL,%1), uzak pencerede açıklama zorunlu; birini gevşetip diğerini unutma tuzağını kapatır), `utils/importedInstallmentPlan.ts`, `utils/importReviewPeriod.ts`, `utils/transactionFingerprint.ts`, `utils/cardControlCenter.ts` (+ test) | `data/repositories/cardsRepo.ts` (`fetchCardExpenseMatchRows`, `fetchCardInstallmentMatchRows`, `fetchCardPaymentMatchRows`, `addCardExpense`, `recordCardInstallmentCarryover`, `payPaymentFromCardImport`, `cancelCardExpense`), `data/repositories/financePanelsRepo.ts` | `components/finance/CurrentMovementImportModal.tsx`, `pages/CardsPage.tsx`, `pages/CardsPage.control.tsx`, `pages/CardsPage.list.tsx` |
| **Taksit takvimi** | `utils/cardInstallmentCalendar.ts`, `utils/importedInstallmentPlan.ts` | `data/repositories/cardsRepo.ts` (`addCardExpense`, `recordCardInstallmentCarryover`; `due_month` legacy ad, gerçek taksit tarihini taşır) | `pages/CardsPage.tsx`, `pages/CardsPage.expense.tsx`, `components/finance/StatementImportModal.tsx`, `components/finance/CurrentMovementImportModal.tsx` |
...
| **Analiz / raporlar** | `utils/analysisView.ts`, `utils/spendingAnomalies.ts`, `utils/subscriptions.ts`, `utils/priceIncreaseRadar.ts`, `utils/yearEndReport.ts` (+ tests) | `data/repositories/analysisRepo.ts` | `pages/AnalysisHub.tsx`, `pages/AnalysisPage.tsx`, `pages/AnalysisDetailPage.tsx` |
...
| **Arabalarım (araç gideri)** | `utils/carExpenses.ts` (+ test; araç başına toplam/bu-ay/kategori kırılımı; kart-etiketli + kart-dışı manuel giderleri birleştirir, kaynaklar ayrık olduğu için çift saymaz — SAF, para yazmaz), `app/useCars.ts` | `data/repositories/carsRepo.ts` (cars/car_expenses CRUD + `setCardExpenseCar` kart etiketleme; `cardsRepo.addCardExpense`/carryover opsiyonel `carId`), `supabase/migrations/20260804120000_add_cars.sql` (`cars` + `car_expenses` + `card_expenses.car_id`; car_expenses net değer/nakit akışına GİRMEZ) | `pages/CarsPage.tsx` (`/analiz/araclar`: araç yönet + kart-dışı gider + kart harcaması ata + araç başına özet), `pages/CardsPage.expense.tsx` (Hızlı harcama formunda araç seçici) |
...
| **Harcanabilir tutar (tek sayı)** | `utils/safeToSpend.ts` (+ test; likit + kalan gelir − kalan yükümlülük − tampon) | `financeSnapshotRepo.ts` | `components/dashboard/SafeToSpendCard.tsx` (dashboard tepesi) |
