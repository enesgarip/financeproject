...
export function SimpleModal({ title, open, children, onClose }: SimpleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end bg-slate-950/45 px-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6">
      <section
        role="dialog"
        aria-modal="true"
        aria-labelledby="simple-modal-title"
        className="max-h-[92svh] w-full min-w-0 overflow-x-hidden overflow-y-auto rounded-t-lg border border-border/85 bg-card text-card-foreground shadow-[var(--shadow-elevated)] sm:max-w-xl sm:rounded-lg"
      >
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-border/80 bg-card/95 px-4 py-3 backdrop-blur">
          <h2 id="simple-modal-title" className="text-base font-black text-foreground">{title}</h2>
          <button
            type="button"
            onClick={onClose}
