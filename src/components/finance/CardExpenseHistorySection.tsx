      ...
      <div className="space-y-2 px-4 py-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="text-xs font-bold text-ink-muted">App dönem harcamaları</p>
            <p className="mt-0.5 text-[11px] text-ink-muted">{periodLabel || 'Dönem belirlenemedi'}</p>
          </div>
          <div className="shrink-0 text-right">
            <p className="text-xs font-black text-ink">{formatAmount(activeTotal)}</p>
            <p className="text-[10px] font-bold text-ink-muted">{expenses.length} kayıt</p>
          </div>
        </div>

        {sortedExpenses.length === 0 ? (
          <p className="rounded-lg bg-page p-2.5 text-[11px] font-medium text-ink-muted">
            Bu dönem için app'te harcama kaydı yok.
          </p>
        ) : (
          <div className="max-h-52 overflow-y-auto rounded-lg border border-line-strong">
            {sortedExpenses.map((expense) => (
              <div
                key={expense.id}
                className="flex items-start justify-between gap-3 border-b border-line-strong px-3 py-2.5 last:border-b-0"
              >
                <div className="min-w-0">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <p className={`min-w-0 truncate text-xs font-bold ${expense.status === 'cancelled' ? 'text-ink-muted line-through' : 'text-ink'}`}>
                ...
                <span className={`shrink-0 text-right text-xs font-black ${expense.status === 'cancelled' ? 'text-ink-muted line-through' : 'text-ink'}`}>
