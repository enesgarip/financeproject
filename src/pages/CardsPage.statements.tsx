...
export function ProvisionPanel({
  ...
  const pendingCardIds = useMemo(() => new Set(pending.map((expense) => expense.card_id)), [pending])
  const pendingCountsByCard = useMemo(() => {
    const counts = new Map<string, number>()
    pending.forEach((expense) => counts.set(expense.card_id, (counts.get(expense.card_id) ?? 0) + 1))
    return counts
  }, [pending])
    ...
    <SurfaceCard className="border-warning/20">
      ...
      <CardContent className="space-y-2 pt-2">
        {filterCards.length > 1 ? (
          <label className="flex items-center justify-between gap-3 rounded-xl border border-line bg-page px-3 py-2 text-xs font-semibold text-ink-muted">
            Kart filtresi
            <select
              aria-label="Provizyonları karta göre filtrele"
              value={activeCardId}
              onChange={(event) => setSelectedCardId(event.target.value)}
              className="min-w-0 max-w-[70%] rounded-lg border border-line-strong bg-raised px-2.5 py-1.5 text-xs font-semibold text-ink"
            >
