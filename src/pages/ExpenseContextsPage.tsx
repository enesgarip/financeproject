import { CreditCard, FolderKanban, PawPrint, Plus, Trash2, Wallet } from 'lucide-react'
...
import { PET_EXPENSE_CATEGORIES, PROJECT_EXPENSE_CATEGORIES, type ExpenseContextSummary } from '../utils/expenseContexts'
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">Evcil hayvan giderlerini veya düğün, taşınma gibi süreli projeleri tek mercekte izle.</p>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        {contexts.length ? (
          <ul className="grid gap-2 sm:grid-cols-2">
            {contexts.map((context) => (
              <li key={context.id} className="flex items-center justify-between rounded-xl border border-border px-3 py-2">
                <div className="flex min-w-0 items-center gap-2">
                  {context.kind === 'pet' ? <PawPrint className="size-4 text-primary" /> : <FolderKanban className="size-4 text-primary" />}
                  <div className="min-w-0"><p className="truncate text-sm font-bold">{context.name}</p><p className="text-xs text-muted-foreground">{context.kind === 'pet' ? 'Evcil hayvan' : 'Etkinlik / Proje'}</p></div>
                </div>
...
function ManualContextExpenseForm({ contexts, userId, onChanged, onError }: {
  ...
  const categories = active?.kind === 'pet' ? PET_EXPENSE_CATEGORIES : PROJECT_EXPENSE_CATEGORIES
  ...
  return (
    <Card>
      ...
      <CardContent><form onSubmit={submit} className="grid gap-2 sm:grid-cols-2 lg:grid-cols-6">
        <Select value={active?.id ?? ''} onChange={(event) => { setContextId(event.target.value); const next = contexts.find((c) => c.id === event.target.value); setCategory((next?.kind === 'pet' ? PET_EXPENSE_CATEGORIES : PROJECT_EXPENSE_CATEGORIES)[0]) }} aria-label="Gider bağlamı">
